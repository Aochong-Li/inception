#!/bin/bash
# Inception (architect-using) for 4 new think-mode models, single trial each.
# GPU-serialized: each cell loads vLLM once, runs, exits; GPU freed between cells.
# Hyperparameters mirror scripts/incept.sh (lio_dev parity); per-model max_tokens
# is sourced from config/target_models.yaml.
set -e
