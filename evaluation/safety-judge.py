...
class SafetyEvaluator:
    def __init__(
        ...
        client_name: str = "openai",  # API client to use ("openai", "deepseek", "togetherai", "openrouter")
    ...
    def _parse_json_responses(self, output_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'response' not in output_df.columns:
            logger.warning("Warning: 'response' column not found in output_df. Returning original dataframe.")
            return output_df
        ...
        for idx, row in output_df.iterrows():
            ...
            try:
                ...
                if response_str.startswith('[') and response_str.endswith(']'):
                    try:
                        ...
                        if isinstance(parsed_list, list) and len(parsed_list) > 0:
                            ...
                            response_str = parsed_list[0] if isinstance(parsed_list[0], str) else str(parsed_list[0])
                            response_str = str(response_str).strip()
                ...
                if json_start >= 0 and json_end > json_start:
                    ...
                    try:
                        json_str = codecs.decode(response_str, 'unicode_escape')
                    except (UnicodeDecodeError, ValueError):
                ...
                if idx < 5:  # Only print first few errors to avoid spam
                    ...
                    print(f"  Response string (first 200 chars): {str(response_str)[:200]}")
    ...
    def _explode_reasoning_traces(self, df: pd.DataFrame) -> pd.DataFrame:
        if self.reasoning_trace_col in df.columns:
            has_lists = df[self.reasoning_trace_col].apply(lambda x: isinstance(x, (list, tuple))).any()
            if has_lists:
                df = df.explode(self.reasoning_trace_col, ignore_index=False)
        else:
            raise ValueError(f"Column {self.reasoning_trace_col} not found in input dataframe")
        return df

    def _check_if_result_exists(self, category: str, overwrite: bool = False) -> bool:
        cache_filepath = os.path.join(self.output_dir, f"{self.nick_name}_safety_judge.pickle")
    ...
    def _batch_traces_by_category(self):
        ...
        return {
            cat: self.input_df[self.input_df[self.category_col] == self.category_labels[cat]]
    ...
    async def _evaluate_by_category(self, batch: pd.DataFrame, category: str, overwrite: bool = False):
        ...
        engine = OpenAI_Engine(
            ...
            batch_io_root=str(Path.home()) + "/inception/evaluation/batch_io",
            cache_filepath=os.path.join(self.output_dir, f"{category}_safety_judge_raw.pickle"),
    ...
    def _merge_results(self) -> pd.DataFrame:
        ...
        for category in self.category_labels.keys():
            pickle_path = os.path.join(self.output_dir, f"{category}_safety_judge_raw.pickle")
            if os.path.exists(pickle_path):
                ...
                if 'response' not in df.columns:
                    logger.warning(f"Category '{category}' result file missing 'response' column")
                    skipped_categories.append(category)
                    continue
                ...
                logger.warning(f"Category '{category}' result file not found at {pickle_path}")
                skipped_categories.append(category)
        
        # Log skipped categories but don't raise error - some categories may legitimately be empty
        if skipped_categories:
            logger.info(f"Skipped categories (no data or missing files): {skipped_categories}")
        ...
        if dataframes:
            ...
            merged_df = merged_df.merge(self.eval_df, left_index=True, right_index=True, how='left', suffixes=('', '_eval'))
    ...
    async def run(self, overwrite: bool = False) -> pd.DataFrame:
        ...
        for category in self.category_labels.keys():
            ...
            coro = self._evaluate_by_category(self.batches[category], category, overwrite=overwrite)
            coroutines.append(coro)
        
        logger.info(f"Running safety evaluation using {self.eval_model} via {self.client_name}")
        ...
        combined_path = os.path.join(self.output_dir, f"{self.nick_name}_safety_judge.pickle")
    ...
    def concatenate_results(self) -> pd.DataFrame:
        ...
        self.input_df = self.input_df.merge(self.eval_df, left_index=True, right_index=True, how='left')
  ...
  python safety-judge.py \\
    ...
    if args.max_rows is not None and args.max_rows < len(input_df):
        if args.random_seed is not None:
            input_df = input_df.sample(frac=1, random_state=args.random_seed).reset_index(drop=True)
    ...
    if not result_df.empty:
        logger.info(f"Evaluation completed successfully. Results shape: {result_df.shape}")
    else:
